...
impl<R: ReadAtom> AtomIterator<R> {
    ...
    /// Restore a top-level atom that was consumed only to confirm end-of-stream.
    ///
    /// Raw sample reads require a pending atom so iteration can later resume at
    /// its real boundary. Seeking track state after EOF does not recreate that
    /// boundary, so the format reader preserves and restores the last `mdat`.
    pub(crate) fn restore_pending(&mut self, atom: AtomHeader) -> Result<()> {
        if self.pending.is_some() || !self.stack.is_empty() {
            return Err(AtomError::UnexpectedReadOperation);
        }
