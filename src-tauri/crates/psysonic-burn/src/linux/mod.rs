...
/// Tracks in the disc's table of contents.
///
...
fn toc_track_count(device: &ScsiDevice) -> Option<u8> {
    ...
    let read = device.receive(&cdb, &mut buffer, TIMEOUT_QUERY).ok()?;
    scsi::parse_toc_track_count(&buffer[..read.min(buffer.len())])
}
...
pub fn burn(
    ...
    // Every command above came back OK, and that proves nothing: a USB-attached
    // drive has been seen accepting a whole Session-At-Once burn and leaving the
    // disc blank. So ask the disc. There is no second write path on Linux to
    // fall back to, so a disc that received nothing is a failure — reported as
    // one, not as a finished burn.
    if !options.test_write {
        ...
        match scsi::verdict_after_write(status, toc_track_count(&device)) {
            scsi::WriteVerdict::NothingWritten => {
                return Err(NOTHING_WRITTEN_MESSAGE.to_string());
            }
