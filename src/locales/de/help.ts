export const help = {
  ...
  q46: 'Kann ich mehrere Server gleichzeitig nutzen?',
  a46: 'Ja. Unter Einstellungen → Server liegen alle Profile, und der Bibliotheks-Bereich in der Seitenleiste bestimmt, welche davon du gerade durchsuchst — wähle Musikordner mehrerer Server zugleich aus, und Psysonic führt sie in der von dir gesetzten Priorität zu einem Katalog zusammen. Mainstage, Alben, Interpreten, Komponisten, Genres, Favoriten, Playlists, Ordner-Browser, Suche, Meistgespielt und Statistiken fassen die gewählten Server zusammen, und Musik, die du auf mehreren Servern hast, erscheint einmal statt pro Server. Eine Warteschlange darf Tracks verschiedener Server mischen; jeder Track holt Stream, Cover, Lyrics und ReplayGain von dem Server, dem er gehört.',
  ...
  a5: 'Gapless puffert den nächsten Track vor, sodass keine Stille zwischen Songs entsteht (ideal für Live-Alben und DJ-Mixe). Crossfade blendet den aktuellen Track aus, während der nächste eingeblendet wird (1–10 s, gut für gemischte Mixe). AutoDJ ist Crossfade, das der Musik folgt: es schneidet die Stille an den Track-Rändern weg und nimmt die Überlappung aus der Inhaltsanalyse, sodass die Überblendung dort sitzt, wo die Songs wirklich enden und beginnen. Nur eines der drei kann aktiv sein — eines einschalten schaltet die anderen aus. Umschalten über die Warteschlangen-Leiste, konfigurieren in Einstellungen → Audio.',
  q6: 'Was ist die Infinite Queue?',
  a6: 'Wenn die Warteschlange leerläuft und Wiederholen aus ist, hängt Psysonic ähnliche / zufällige Tracks aus deiner Bibliothek hinten dran, sodass die Wiedergabe nie stoppt. Auto-hinzugefügte Tracks erscheinen unter dem Trenner „— Automatisch hinzugefügt —". Umschalten über das Unendlichkeits-Icon in der Warteschlangen-Leiste oder im Mini-Player.',
  ...
  q47: 'Timeline-Ansicht der Warteschlange und „Ab hier abspielen“?',
  a47: 'Einstellungen → Personalisierung bietet für die Warteschlange eine Timeline-Ansicht: der aktuelle Track bleibt oben, das bereits Gehörte scrollt darüber, das Kommende steht darunter. Ein Rechtsklick auf einen vergangenen Eintrag und „Ab hier abspielen“ stellt diesen Moment wieder her — den Track, alle späteren Verlaufseinträge und die bestehende Als-Nächstes-Liste, in ihrer ursprünglichen Reihenfolge und von den Servern, von denen sie kamen.',
  ...
  q48: 'Kann ich die Streaming-Bitrate begrenzen?',
  a48: 'Pro Serveradresse, unter Einstellungen → Server beim Bearbeiten eines gespeicherten Navidrome-Profils. Wähle keine Begrenzung oder eine Obergrenze von 320 bis 64 kbit/s, dazu Auto, MP3, Opus oder AAC als Zielformat — Navidrome transkodiert den Live-Stream entsprechend. Weil die Einstellung an der Adresse hängt, können ein LAN-Zugang und ein öffentlicher Zugang unterschiedliche Profile verwenden, und die Wiedergabe folgt dem gerade verbundenen. Offline-Downloads, synchronisierte Favoriten und der Hot Cache holen immer die Originaldatei, gespeicherte Musik behält also ihre volle Qualität.',
  ...
  q49: 'Raster oder Tabelle auf den Album-Seiten?',
  a49: 'Alle Alben, Neuerscheinungen und Lossless lassen sich zwischen Kachel-Raster und Tabelle umschalten, und jede Seite merkt sich ihre eigene Wahl. Die Tabelle zeigt Cover, Titel, Interpret, Anzahl der Songs, Jahr, Dauer und das Hinzufügedatum, sortiert per Spaltenkopf nach Titel oder Jahr und blendet Spalten von rechts aus, wenn das Fenster schmaler wird.',
  q50: 'Meine Playlists und die von anderen geteilten?',
  a50: 'Ein Subsonic-Server liefert deine eigenen Playlists und alle öffentlichen Playlists aller anderen in einer einzigen Liste. Die Playlists-Seite kann das aufteilen: deine eigenen, die von dir geteilten und die, die andere mit dir teilen — der Umschalter erscheint, sobald auf dem Server tatsächlich etwas geteilt ist, und arbeitet mit der Ordneransicht und der Suche zusammen. Die Seitenleiste zeigt zu jeder Playlist das Cover und die Anzahl der Songs; Seitenleiste und Seite lassen sich nach Name, Erstelldatum oder Songanzahl sortieren.',
  q51: 'Was ist die Komponisten-Seite?',
  a51: 'Komponisten (Seitenleiste) listet die Komponisten aus deinen Bibliothek-Tags und öffnet pro Komponist eine Detailseite mit den Werken, bei denen er genannt ist. Sie folgt demselben Bibliotheks-Bereich wie der Rest der App — bei mehreren gewählten Servern also als eine zusammengeführte Liste.',
  // ── Section 5: Lyrics ──────────────────────────────────────────────────
  s5: 'Lyrics',
  ...
  a52: 'Einstellungen → Darstellung → Visualizer schaltet ihn für „Aktuelle Wiedergabe“, für den Vollbild-Player oder für beides ein — jeder hat seinen eigenen Schalter. Wähle Spektrum, Oszilloskop, Radial-Scope oder Stereofeld, nimm die Farben aus dem Cover oder aus deinem Theme und stelle Empfindlichkeit, Reaktion, Bildrate und Peak-Marker ein. Internetradio ist eingeschlossen, solange dessen Equalizer-Graph aktiv ist.',
  q53: 'Wo bekomme ich weitere Themes?',
  a53: 'Einstellungen → Themes enthält den Theme Store: er listet Community-Themes, zeigt zu jedem eine Vorschau und installiert, aktualisiert oder entfernt sie direkt. Katalog und Vorschauen kommen von GitHub, dabei werden keine persönlichen Daten gesendet. Installierte Themes stehen anschließend neben den mitgelieferten in derselben Auswahl.',
  q54: 'Kann ich die Hintergrundbilder ändern?',
  a54: 'Unter Einstellungen → Integrationen → Hintergründe legst du fest, welches Bild die Mainstage, der Kopfbereich der Interpretenseite und der Vollbild-Player verwenden und in welcher Reihenfolge die Quellen probiert werden — Banner, Fanart-Interpretenbild oder das Bild von deinem Server. Zum Umsortieren ziehen, oder eine Quelle abschalten, damit sie übersprungen wird.',
  ...
  a33: 'Smart Playlists sind dynamische Playlists auf Navidrome-Seite, die durch Regeln definiert werden. Psysonic erkennt sie an den nativen Playlist-Metadaten (einem gültigen all/any-Regeldokument), nicht an einem Namenspräfix. Falls die native Playlist-API nicht verfügbar ist, gelten bestehende Namen mit psy-smart- weiterhin als smart. Öffne auf der Playlists-Seite „Regeln bearbeiten“ für einen Basis-/Erweitert-/JSON-Editor. Neue Playlists behalten den eingegebenen Namen. Bestehende Namen mit psy-smart- bleiben sichtbar, bis du sie umbenennst. Tracklisten sind schreibgeschützt, weil Navidrome die Regeln auswertet; nutze die ID-Sortierung, um die Serverreihenfolge beizubehalten. Du kannst weiterhin Titel abspielen, zur Warteschlange hinzufügen, Metadaten bearbeiten, Playlists löschen und einen vorhandenen Offline-Cache entfernen. Erfordert Navidrome 0.48+.',
  q55: 'Wie funktionieren die Smart-Playlist-Modi Basis, Erweitert und JSON?',
  a55: 'Der Basis-Modus behält Psysonics Genre-, Jahres-, Bewertungs- und einfache Textfilter bei, wenn sich die Regeln exakt darstellen lassen. Verschachtelte Gruppen, benutzerdefinierte Felder, zusätzliche Sortierungen oder unbekannte Klauseln öffnen sich unter Erweitert, damit nichts verloren geht; Basis bleibt gesperrt, bis das Dokument verlustfrei dargestellt werden kann. Erweitert listet die freigegebenen Navidrome-Spaltenfelder sowie standardmäßig importierte Tags und Rollen auf (mood, grouping, composer, conductor und den Rest aus mappings.yaml). Serverspezifische benutzerdefinierte Tags und Rollen werden unter Einstellungen → Bibliothek → Benutzerdefinierte Smart-Playlist-Felder hinzugefügt und erscheinen danach in der erweiterten Feldauswahl. JSON bearbeitet das Navidrome-Rohdokument, meldet Strukturfehler und warnt vor nicht unterstützten oder versionsabhängigen Pfaden. „JSON-Vorschau“ wechselt zu dieser Dokumentansicht. „Passende Titel als Vorschau“ zeigt eine einmalige Auswahl und keine live aktualisierte Ergebnisliste. „Auf Serverstand zurücksetzen“ lädt die gespeicherten Regeln neu; „Kopie speichern“ erstellt eine neue Playlist, ohne das Original zu überschreiben.',
  q56: 'Welche Navidrome-Versionen schalten Smart-Playlist-Funktionen frei?',
  a56: 'Grundlegende Smart Playlists benötigen Navidrome 0.48+. Regeln zur Playlist-Zugehörigkeit benötigen 0.52+. Dynamische Tags und Rollen benötigen 0.55+. Mehrere Sortierschlüssel benötigen 0.57+. Prozentuale Limits und Album-/Künstlerfelder benötigen 0.61+. Anwesenheitsoperatoren (fehlt / ist vorhanden) benötigen 0.62+, die erweiterte Feldunterstützung 0.63+. Nach dem Speichern lädt Psysonic die gespeicherten Regeln erneut und warnt, wenn der Server Klauseln verworfen hat. Bei Navidrome 0.63.x kann es bis zum Ablauf des Aktualisierungsfensters dauern, bevor materialisierte Titel aktualisiert werden — das ist eine Serveraktualisierung und kein Hintergrundjob in Psysonic.',
  q34: 'Einstellungen sichern und wiederherstellen?',
  a34: 'Einstellungen → Storage → Backup & Restore exportiert Server-Profile, Themes, Tastenkürzel und andere Einstellungen in eine einzige JSON-Datei. Auf einem anderen Gerät oder nach einer Neuinstallation importieren, um alles auf einmal wiederherzustellen. Server-Passwörter sind enthalten — Datei sicher aufbewahren. Heruntergeladene Musik, Zwischenspeicher und ein angeschlossenes Sync-Gerät sind nicht enthalten. Ein Theme mit eigenen Bilddateien muss neu installiert werden.',
