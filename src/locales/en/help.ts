export const help = {
  ...
  a33: 'Smart Playlists are Navidrome-side dynamic playlists defined by rules. Psysonic identifies them from native playlist metadata (a valid all/any rules document), not a name prefix. If the native playlist API is unavailable, existing psy-smart- names still count as smart. Open Edit Rules on the Playlists page for a Basic / Advanced / JSON editor. New playlists keep the name you type. Existing psy-smart- names stay visible until you rename them. Track lists are read-only because Navidrome evaluates the rules; use ID sort to keep that server order. You can still play, queue, edit metadata, delete, and remove an existing offline cache. Requires Navidrome 0.48+.',
  q55: 'How do Basic, Advanced, and JSON smart-playlist modes work?',
  a55: 'Basic keeps Psysonic’s genre, year, rating, and simple text filters when those rules can be shown exactly. Nested groups, custom fields, extra sorts, or unknown clauses open in Advanced so nothing is dropped; Basic stays unavailable until the document can be projected without loss. Advanced lists released Navidrome column fields plus default imported tags and roles (mood, grouping, composer, conductor, and the rest of mappings.yaml). Server-specific custom tags and roles are added in Settings → Library → Smart Playlist Custom Fields, then appear in the Advanced field picker. JSON edits the raw Navidrome document, reports structural errors, and warns about unsupported or version-gated paths. Preview JSON switches to that document view. Preview matching tracks is a one-shot sample, not a live result list. Reset to Server reloads the saved rules; Save a copy creates a new playlist without overwriting the original.',
  q56: 'Which Navidrome versions unlock smart-playlist features?',
  a56: 'Base smart playlists need Navidrome 0.48+. Playlist membership rules need 0.52+. Dynamic tags and roles need 0.55+. Multiple sort keys need 0.57+. Percentage limits and album/artist fields need 0.61+. Presence operators (is missing / is present) need 0.62+, with expanded field support on 0.63+. After save, Psysonic refetches the stored rules and warns if the server dropped clauses. Navidrome 0.63.x may wait a refresh-delay window before materialized tracks update — that is a server refresh, not a background job in Psysonic.',
  q34: 'Backup and restore settings?',
  a34: 'Settings → Storage → Backup & Restore exports server profiles, themes, keybindings and other settings to a single JSON file. Import the file on another machine or after a reinstall to restore them all at once. Server passwords are included — keep the file private. Downloaded music, caches and a connected sync device are not included. A theme that ships its own image files has to be installed again.',
