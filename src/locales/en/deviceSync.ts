export const deviceSync = {
  title: 'Device Sync',
  targetFolder: 'Target Folder',
  noFolderChosen: 'No folder chosen',
  selectDrive: 'Select a drive…',
  refreshDrives: 'Refresh drives',
  noDrivesDetected: 'No removable drives detected',
  browseManual: 'Browse manually…',
  free: 'free',
  notMountedVolume: 'Target is not on a mounted volume. The drive may have been disconnected.',
  deviceChanged: 'The selected device changed or was disconnected. Reconnect the original device and try again.',
  chooseFolder: 'Choose…',
  targetDevice: 'Target Device',
  schemaLabel: 'Naming scheme',
  schemaHint: 'Fixed scheme for reliable cross-OS sync. Playlists are written as .m3u8 that reference the album tracks — no duplicates on the device.',
  selfContainedLayoutHint: 'Each playlist keeps its own track copies, so its folder can be moved or copied independently.',
  sharedLayoutHint: 'Playlist files reference one shared album-tree copy of each track, saving space and avoiding duplicate library entries.',
  flatLayoutHint: 'Every track goes straight into the main folder of the device, for players that cannot browse folders. Playlists are .m3u8 files next to them.',
  layout: 'Layout',
  playlistStorageSelfContained: 'Self-contained folders',
  playlistStorageShared: 'Shared album tracks',
  layoutFlat: 'All files in one folder',
  playlistPathStyle: 'M3U path style',
  playlistPathRelative: 'Relative to playlist',
  playlistPathRooted: 'From device root (/Artist/Album/…)',
  playlistPathAbsolute: 'Absolute (full file paths)',
  localTargetBadge: 'Local folder',
  localTargetTitle: 'Sync to a local folder?',
  localTargetConfirm: '{{path}} is on this computer, not on a removable drive. Psysonic will sync into it like a device and remember it as a local target.',
  localTargetUse: 'Use this folder',
  localTargetInvalid: 'This folder cannot be used as a sync target.',
  localTargetHint: 'For DJ software such as Rekordbox, use shared album tracks with absolute paths: files keep their location when a playlist changes, so imported analysis and cue points stay linked.',
  transcodeFormat: 'Format',
  transcodeOriginal: 'Original files',
  transcodeMp3: 'MP3',
  transcodeAac: 'AAC',
  transcodeOpus: 'Opus',
  transcodeBitrate: 'Max bitrate',
  transcodeBitrateServer: 'Server default',
  transcodeBitrateValue: '{{kbps}} kbps',
  transcodeHint: 'Your server converts the files while syncing, so the format must be enabled in its transcoding settings. Tags are kept, embedded cover art may not be. MP3 works with most DJ software.',
  migrateButton: 'Reorganize existing files…',
  migrateTooltip: 'Rename existing files on the device into the new scheme (from the old filename template).',
  migrateTitle: 'Reorganize existing files',
  migrateLoading: 'Analyzing existing files…',
  migrateNothingToDo: 'All existing files already match the new scheme — nothing to do.',
  migrateNoTemplate: 'No legacy filename template found on the device. Migration only applies when the stick was synced with a Psysonic version that supported custom templates.',
  migrateFilesToRename: 'files will be renamed',
  migrateUnchanged: '{{n}} files are already at the correct path',
  migrateCollisions: '{{n}} files cannot be renamed automatically (multiple tracks map to the same target). They will be left untouched — the next sync re-downloads them into the correct location.',
  migratePreviewNote: 'Old template: {{tpl}}',
  migrateExecuting: 'Renaming files…',
  migrateSuccess: '{{n}} files renamed successfully',
  migrateFailed: '{{n}} renames failed',
  migrateShowErrors: 'Show errors',
  migrateStart: 'Start renaming',
  onDevice: 'Device Manager',
  addSources: 'Add…',
  colName: 'Name',
  colType: 'Type',
  colStatus: 'Status',
  syncResult: '{{done}} transferred, {{skipped}} already up to date ({{total}} total)',
  deleteFromDevice: 'Mark for deletion ({{count}})',
  confirmDelete: 'Delete {{count}} item(s) from the device: {{names}}?',
  deleteComplete: '{{count}} item(s) removed from device.',
  manifestImported: 'Imported {{count}} source(s) from device manifest.',
  legacyRecoveryTitle: 'Unassigned device sources',
  legacyRecoveryDescription: '{{count}} source(s) came from an older configuration without a server owner. Choose the server they belong to or discard them.',
  legacyRecoveryServer: 'Assign to server',
  legacyRecoveryChooseServer: 'Choose a server…',
  legacyRecoveryApply: 'Recover sources',
  legacyRecoveryDiscard: 'Discard',
  legacyRecoveryDiscardConfirm: 'Discard these unassigned device sources?',
  legacyRecoveryPending: 'This server is still completing its ID migration. Recovery will be available afterward.',
  legacyRecoveryOwnerConflict: 'Clear sources assigned to another server before recovering these sources.',
  legacyRecoveryComplete: 'Device sources recovered.',
  legacyRecoveryWriteFailed: 'Could not update the device manifest. Reconnect the device and try again.',
  selectedSources: 'Selected Sources',
  noSourcesSelected: 'No sources selected. Click items in the browser to add them.',
  clearAll: 'Clear all',
  tabPlaylists: 'Playlists',
  tabAlbums: 'Albums',
  tabArtists: 'Artists',
  searchPlaceholder: 'Search…',
  syncButton: 'Sync to Device',
  actionTransfer: 'Transfer to Device',
  actionDelete: 'Delete from Device',
  actionApplyAll: 'Apply All Changes',
  cancel: 'Cancel',
  noTargetDir: 'Please choose a target folder first.',
  noSources: 'Please select at least one source.',
  noTracks: 'No tracks found in the selected sources.',
  fetchError: 'Failed to fetch tracks from server.',
  cleanupFailed: 'Sync finished, but some old files could not be removed. Reconnect the same device and sync again.',
  pathCollision: 'Two tracks resolve to the same device path. Rename the conflicting track or album and try again.',
  syncComplete: 'Sync complete.',
  dismiss: 'Dismiss',
  cancelSync: 'Cancel',
  syncCancelled: 'Sync cancelled ({{done}} / {{total}} transferred).',
  statusSynced: 'Synced',
  statusPending: 'Pending',
  statusDeletion: 'Deletion',
  markForDeletion: 'Mark for deletion',
  undoDeletion: 'Undo deletion',
  removeSource: 'Remove',
  syncInBackground: 'Sync started in background — you can navigate away.',
  syncInProgress: '{{done}} / {{total}} tracks…',
  scanningDevice: 'Scanning device…',
  syncSummary: 'Sync Summary',
  calculating: 'Calculating required payload…',
  filesToAdd: 'Files to Add:',
  filesToDelete: 'Files to Delete:',
  filesToMove: 'Files to Move:',
  netChange: 'Net Change:',
  availableSpace: 'Available Disk Space:',
  spaceWarning: 'Warning: Target device does not have enough reported space.',
  proceed: 'Proceed with Sync',
  notEnoughSpace: 'Not enough physical disk space detected!',
  liveSearch: 'Live',
  randomAlbumsLabel: 'Random Albums',
  serverUnresolved: 'The server for this device configuration is no longer set up. Assign the sources to a server to continue.',
  browserOwnerUnresolved: 'These sources belong to {{server}}, which is no longer set up. Assign them to a server to browse again.',
  browserNoServer: 'No server connected.',
  browserLoadFailed: 'Could not load from the server.',
  browserEmpty: 'Nothing to show here.',
  ownerRepairTitle: 'Server no longer set up',
  ownerRepairDescription: 'This device is bound to {{server}}, which is not among your servers. If it moved to a new address, pick it below to re-bind the device.',
  ownerRepairApply: 'Re-bind device',
  ownerRepairComplete: 'Device re-bound to the selected server.',
  ownerRepairFailed: 'Could not update the device manifest. Reconnect the device and try again.',
  ownerRepairNeedsDevice: 'Connect the device and choose its folder first.',
};
