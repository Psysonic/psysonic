export const deviceSync = {
  title: 'Sync appareil',
  targetFolder: 'Dossier cible',
  noFolderChosen: 'Aucun dossier sélectionné',
  selectDrive: 'Sélectionner un lecteur…',
  refreshDrives: 'Actualiser les lecteurs',
  noDrivesDetected: 'Aucun lecteur amovible détecté',
  browseManual: 'Parcourir manuellement…',
  free: 'libre',
  notMountedVolume: 'La cible n\u0027est pas sur un volume monté. Le lecteur a peut-être été déconnecté.',
  deviceChanged: 'L\u0027appareil sélectionné a changé ou a été déconnecté. Reconnectez l\u0027appareil d\u0027origine et réessayez.',
  chooseFolder: 'Choisir…',
  filenameTemplate: 'Modèle de nom de fichier',
  targetDevice: 'Appareil cible',
  templateHint: 'Variables : {artist}, {album}, {title}, {track_number}, {disc_number}, {year}',
  selfContainedLayoutHint: 'Chaque liste conserve ses propres copies des pistes, afin que son dossier puisse être déplacé ou copié indépendamment.',
  sharedLayoutHint: 'Les fichiers de listes utilisent une seule copie partagée de chaque piste dans l’arborescence des albums, ce qui économise de l’espace et évite les doublons dans la bibliothèque.',
  flatLayoutHint: 'Chaque piste va directement dans le dossier principal de l’appareil, pour les lecteurs qui ne savent pas parcourir les dossiers. Les listes de lecture sont des fichiers .m3u8 à côté.',
  layout: 'Organisation',
  playlistStorageSelfContained: 'Dossiers autonomes',
  playlistStorageShared: 'Pistes d’album partagées',
  layoutFlat: 'Tous les fichiers dans un dossier',
  playlistPathStyle: 'Style des chemins M3U',
  playlistPathRelative: 'Relatifs à la liste',
  playlistPathRooted: 'Depuis la racine de l’appareil (/Artiste/Album/…)',
  playlistPathAbsolute: 'Absolu (chemins complets)',
  localTargetBadge: 'Dossier local',
  localTargetTitle: 'Synchroniser vers un dossier local ?',
  localTargetConfirm: '{{path}} se trouve sur cet ordinateur, pas sur un support amovible. Psysonic y synchronisera comme sur un appareil et le mémorisera comme cible locale.',
  localTargetUse: 'Utiliser ce dossier',
  localTargetInvalid: 'Ce dossier ne peut pas servir de cible de synchronisation.',
  localTargetHint: 'Pour les logiciels DJ comme Rekordbox, utilisez les pistes d’album partagées avec des chemins absolus : les fichiers gardent leur emplacement quand une playlist change, donc l’analyse et les points cue importés restent liés.',
  transcodeFormat: 'Format',
  transcodeOriginal: 'Fichiers d’origine',
  transcodeMp3: 'MP3',
  transcodeAac: 'AAC',
  transcodeOpus: 'Opus',
  transcodeBitrate: 'Débit max.',
  transcodeBitrateServer: 'Valeur du serveur',
  transcodeBitrateValue: '{{kbps}} kbit/s',
  transcodeHint: 'Votre serveur convertit les fichiers pendant la synchronisation : le format doit donc être activé dans ses réglages de transcodage. Les tags sont conservés, la pochette intégrée peut ne pas l’être. Le MP3 fonctionne avec la plupart des logiciels DJ.',
  cleanupButton: 'Supprimer les fichiers absents',
  cleanupNothingToDelete: 'Rien à supprimer — l\'appareil est déjà synchronisé.',
  confirmCleanup: 'Supprimer {{count}} fichier(s) de l\'appareil qui ne font pas partie de la sélection actuelle. Continuer ?',
  cleanupComplete: '{{count}} fichier(s) supprimé(s) de l\'appareil.',
  selectedSources: 'Sources sélectionnées',
  noSourcesSelected: 'Aucune source sélectionnée. Cliquez sur des éléments dans le navigateur pour les ajouter.',
  clearAll: 'Tout effacer',
  tabPlaylists: 'Listes de lecture',
  tabAlbums: 'Albums',
  tabArtists: 'Artistes',
  searchPlaceholder: 'Rechercher…',
  syncButton: 'Synchroniser vers l\'appareil',
  actionTransfer: 'Transférer vers l\'appareil',
  actionDelete: 'Supprimer de l\'appareil',
  actionApplyAll: 'Appliquer toutes les modifications',
  templatePreview: 'Aperçu',
  templatePresetStandard: 'Standard',
  templatePresetMultiDisc: 'Multi-Disc',
  templatePresetAltFolder: 'Dossier alt.',
  tokenSlashHint: '/ = séparateur de dossier',
  cancel: 'Annuler',
  noTargetDir: 'Veuillez d\'abord choisir un dossier cible.',
  noSources: 'Veuillez sélectionner au moins une source.',
  noTracks: 'Aucune piste trouvée dans les sources sélectionnées.',
  fetchError: 'Échec du chargement des pistes depuis le serveur.',
  cleanupFailed: 'La synchronisation est terminée, mais certains anciens fichiers n\u0027ont pas pu être supprimés. Reconnectez le même appareil et relancez la synchronisation.',
  pathCollision: 'Deux pistes correspondent au même chemin sur l\u0027appareil. Renommez la piste ou l\u0027album en conflit et réessayez.',
  syncComplete: 'Synchronisation terminée.',
  dismiss: 'Fermer',
  cancelSync: 'Annuler',
  syncCancelled: 'Synchronisation annulée ({{done}} / {{total}} transférés).',
  colName: 'Nom',
  colType: 'Type',
  colStatus: 'Statut',
  onDevice: 'Gestionnaire d\'appareil',
  addSources: 'Ajouter…',
  syncResult: '{{done}} transféré(s), {{skipped}} déjà à jour ({{total}} au total)',
  deleteFromDevice: 'Marquer pour suppression ({{count}})',
  confirmDelete: 'Supprimer {{count}} élément(s) du périphérique : {{names}} ?',
  deleteComplete: '{{count}} élément(s) supprimé(s) du périphérique.',
  manifestImported: '{{count}} source(s) importée(s) depuis le manifeste du périphérique.',
  legacyRecoveryTitle: 'Sources du périphérique non attribuées',
  legacyRecoveryDescription: '{{count}} source(s) proviennent d’une ancienne configuration sans serveur propriétaire. Choisissez leur serveur ou supprimez-les.',
  legacyRecoveryServer: 'Attribuer au serveur',
  legacyRecoveryChooseServer: 'Choisir un serveur…',
  legacyRecoveryApply: 'Récupérer les sources',
  legacyRecoveryDiscard: 'Supprimer',
  legacyRecoveryDiscardConfirm: 'Supprimer ces sources de périphérique non attribuées ?',
  legacyRecoveryPending: 'Ce serveur termine encore la migration de ses identifiants. La récupération sera ensuite disponible.',
  legacyRecoveryOwnerConflict: 'Effacez les sources attribuées à un autre serveur avant de récupérer celles-ci.',
  legacyRecoveryComplete: 'Sources du périphérique récupérées.',
  legacyRecoveryWriteFailed: 'Impossible de mettre à jour le manifeste du périphérique. Reconnectez-le et réessayez.',
  statusSynced: 'Synchronisé',
  statusPending: 'En attente',
  statusDeletion: 'Suppression',
  markForDeletion: 'Marquer pour suppression',
  removeSource: 'Supprimer',
  undoDeletion: 'Annuler la suppression',
  syncInBackground: 'Sync démarré en arrière-plan — vous pouvez naviguer ailleurs.',
  syncInProgress: '{{done}} / {{total}} pistes…',
  syncSummary: 'Résumé de la synchronisation',
  calculating: 'Calcul de la charge utile…',
  filesToAdd: 'Fichiers à ajouter :',
  filesToDelete: 'Fichiers à supprimer :',
  filesToMove: 'Fichiers à déplacer :',
  netChange: 'Variation nette :',
  availableSpace: 'Espace disque disponible :',
  spaceWarning: 'Attention : L\'appareil cible ne dispose pas d\'assez d\'espace signalé.',
  proceed: 'Procéder à la synchronisation',
  notEnoughSpace: 'Espace disque physique insuffisant détecté !',
  liveSearch: 'Live',
  randomAlbumsLabel: 'Albums aléatoires',
  serverUnresolved: 'Le serveur de cette configuration d’appareil n’est plus configuré. Attribuez les sources à un serveur pour continuer.',
  browserOwnerUnresolved: 'Ces sources appartiennent à {{server}}, qui n’est plus configuré. Attribuez-les à un serveur pour parcourir à nouveau.',
  browserNoServer: 'Aucun serveur connecté.',
  browserLoadFailed: 'Impossible de charger depuis le serveur.',
  browserEmpty: 'Rien à afficher ici.',
  ownerRepairTitle: 'Serveur plus configuré',
  ownerRepairDescription: 'Cet appareil est lié à {{server}}, qui ne fait plus partie de vos serveurs. S’il a changé d’adresse, sélectionnez-le ci-dessous pour relier l’appareil.',
  ownerRepairApply: 'Relier l’appareil',
  ownerRepairComplete: 'Appareil relié au serveur sélectionné.',
  ownerRepairFailed: 'Impossible de mettre à jour le manifeste de l’appareil. Reconnectez-le et réessayez.',
  ownerRepairNeedsDevice: 'Connectez l’appareil et choisissez d’abord son dossier.',
};
