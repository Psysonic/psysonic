export const deviceSync = {
  ...
  flatLayoutHint: '每首曲目直接放在设备的主文件夹中，适用于无法浏览文件夹的播放器。播放列表以 .m3u8 文件放在旁边。',
  layout: '布局',
  playlistStorageSelfContained: '独立文件夹',
  playlistStorageShared: '共享专辑曲目',
  layoutFlat: '所有文件放在一个文件夹中',
  ...
  transcodeAac: 'AAC',
  transcodeOpus: 'Opus',
  transcodeBitrate: '最大码率',
  transcodeBitrateServer: '服务器默认',
  transcodeBitrateValue: '{{kbps}} kbps',
  transcodeHint: '服务器会在同步时转换文件，因此需要在其转码设置中启用该格式。标签会保留，内嵌封面可能不会保留。MP3 适用于大多数 DJ 软件。',
  ...
  serverUnresolved: '此设备配置的服务器已不再存在。请将来源分配到某个服务器后继续。',
  browserOwnerUnresolved: '这些来源属于 {{server}}，该服务器已不再存在。请将它们分配到某个服务器以继续浏览。',
  browserNoServer: '未连接服务器。',
  browserLoadFailed: '无法从服务器加载。',
  browserEmpty: '这里没有可显示的内容。',
  ownerRepairTitle: '服务器已不再存在',
  ownerRepairDescription: '此设备绑定到 {{server}}，但它不在你的服务器列表中。如果它换了新地址，请在下方选择以重新绑定设备。',
  ownerRepairApply: '重新绑定设备',
  ownerRepairComplete: '设备已绑定到所选服务器。',
  ownerRepairFailed: '无法更新设备清单。请重新连接设备后再试。',
  ownerRepairNeedsDevice: '请先连接设备并选择其文件夹。',
};
