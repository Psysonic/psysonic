export const settings = {
  ...
  discordCoverDesc: '你的 Discord 个人资料可以显示的专辑封面来源。「无（仅应用图标）」不会发布任何内容。其他选项会将图片链接发送到 Discord 的图片代理——查看你个人资料的人可能会看到你服务器的公开地址，而服务器封面需要可公开访问的服务器。下方应用内的封面来源与此无关。',
  discordCoverNone: '无（仅显示应用图标）',
  discordCoverServer: '跟随应用内来源',
  discordCoverApple: '仅外部来源（不含服务器链接）',
  coverSourceSectionTitle: '专辑封面',
  coverSourceTitle: '专辑封面来源',
  coverSourceDesc: '当服务器没有封面时从何处获取。按从上到下的顺序使用，第一个返回图片的已启用来源生效。拖动可重新排序，关闭某个来源即可跳过。',
  coverSourceServer: '服务器（专辑信息）',
  ...
  lyricsPronunciationTitle: '发音',
  lyricsRomanization: '发音与日语罗马字',
  lyricsRomanizationDesc: '在歌词下显示服务器提供的发音；若无可用发音，则在本地为日语文本生成赫本式罗马字。',
  lyricsWordHighlightTitle: '逐词高亮',
  lyricsWordHighlightStep: '分步',
  ...
  playQueueSyncTitle: '同步播放队列',
  playQueueSyncDesc: '将此设备的队列和播放位置与服务器同步，以便在其他设备上继续播放。关闭后，队列只保留在此设备上。',
  integrationsPrivacyBody: '此标签页中的所有集成均为 <strong>可选</strong>，启用后会向外部服务或您的 Navidrome 服务器发送数据。Last.fm 接收您的收听历史，Discord 在您的个人资料中显示正在播放的曲目，Bandsintown 会按艺人查询以获取巡演日期，"正在播放" 分享会向您 Navidrome 服务器的其他用户公开您当前的曲目。如果您不需要这些功能，只需将相应部分保持停用即可。',
  ...
  favoritesLayoutTitle: '收藏夹页面板块',
  favoritesLayoutDesc: '拖动以重新排序，切换以隐藏收藏夹页面的各个板块。没有收藏内容的板块会自动跳过。',
  favoritesLayoutReset: '重置为默认',
  ...
  smartPlaylistCustomFieldsTitle: '智能播放列表自定义字段',
  smartPlaylistCustomFieldsDesc: '为高级智能播放列表字段选择器添加服务器特有的标签和角色。Navidrome 内置字段以及 mappings.yaml 的默认标签已经列出。需要 Navidrome 0.55+。',
  smartPlaylistCustomFieldsList: '自定义字段',
  smartPlaylistCustomFieldsEmpty: '尚未添加自定义字段。',
  smartPlaylistCustomFieldsInvalid: '请使用以字母开头且不含空格的名称（例如 mood_energy）。不能使用 sort 或 limit 等保留名称。',
  smartPlaylistCustomFieldsDuplicate: '该字段已在列表中。',
  smartPlaylistCustomFieldsRemove: '移除 {{name}}',
