...
const ARTWORK_PREFIXES = ['mf-', 'al-', 'ar-', 'pl-', 'ra-', 'tr-', 'dc-'] as const;
...
/** Canonicalize a typed Navidrome entity ID without consulting the server. */
...
/** Canonicalize only the entity-bearing payload of a typed Navidrome artwork ID. */
export function canonicalNavidromeArtworkId(value: string): string {
  const prefix = ARTWORK_PREFIXES.find(candidate => value.startsWith(candidate));
