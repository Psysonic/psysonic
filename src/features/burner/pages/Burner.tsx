...
export default function Burner() {
  ...
  const handleBurn = useCallback(async () => {
    ...
    // Worked out once: the drive is asked for exactly what the store remembers
    // asking for, so a disc that comes back without CD-TEXT can be explained.
    const cdTextRequested = settings.cdText && cdTextSupported;
    useBurnJobStore.getState().start(jobId, layout.totalSectors, settings.testWrite, cdTextRequested);
