 ...
 * carry the same text. It is `null` when the title already contains it, because
...
export function deriveAlbumVersion(
  ...
  const bare = version.replace(/^[([{<](.*)[)\]}>]$/, '$1').trim().toLowerCase();
  if (bare && album.name.toLowerCase().includes(bare)) return null;
  return version;
}
